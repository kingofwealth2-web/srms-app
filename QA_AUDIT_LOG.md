...
- **Not live-tested (destructive):** a real end-to-end year rollover. Safe by design — the rollover is `SECURITY DEFINER` (owner `postgres`, exempted by the trigger's role check) AND sets `srms.year_rollover=on` (second bypass). Recommend one real rollover on staging (or a throwaway school) for final confidence before relying on it in prod.
