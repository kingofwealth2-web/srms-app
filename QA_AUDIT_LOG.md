...
### [F-014] Past-year read-only and plan gating are UI-only (not server-enforced) — 🔵 Low (within-tenant) — ✅ fixed & verified on staging (pending prod)
...
- **Verified on staging (authenticated superadmin, 2026-08-07):** past-year grade insert BLOCKED with the intended message; current-year insert OK; dash-format `2026-2027` also OK (normalization holds → fresh schools not locked out). Plan caps: functions return null under pro, 80/2 under starter; new active student BLOCKED at 80; `create_auth_user` BLOCKED at 2; editing an existing active student under starter still OK; plan restored, no leftover rows.
- **Rollover live-tested on staging (2026-08-07):** ran a real `start-new-year` (2026/2027 → 2027/2028) via the edge function. Completed in ~2.1s, NOT blocked by the year-lock — carried 15 unpaid balances as arrears (F-011), enrolled all 380 active students into the new year, copied 27 fee templates, advanced `settings.academic_year`. Post-rollover the lock tracks the new current year: writes to 2027/2028 OK, writes to the now-archived 2026/2027 BLOCKED. (Staging school is now on 2027/2028 — rollovers don't auto-reverse.)
