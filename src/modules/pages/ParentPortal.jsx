...
<div style={{ fontSize: 32 }}>Hello</div>
    ...
    <div style={{ maxWidth: 1100, margin: '0 auto', height: 60, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 16 }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
        <button onClick={() => setIsDark(d => !d)} title={isDark ? 'Light mode' : 'Dark mode'}
          ...
          <span style={{ position: 'absolute', left: 6, fontSize: 10, opacity: isDark ? 0.35 : 1, lineHeight: 1, top: '50%', transform: 'translateY(-50%)' }}>Sun</span>
          <span style={{ position: 'absolute', right: 6, fontSize: 9, opacity: isDark ? 1 : 0.3, lineHeight: 1, top: '50%', transform: 'translateY(-50%)' }}>Moon</span>
  ...
  <div style={{ maxWidth: 1100, margin: '0 auto', padding: '24px 24px' }}>
    ...
    <div style={{ display: 'flex', gap: 4, marginBottom: 24, background: 'var(--ink2)', padding: 4, borderRadius: 10, border: '1px solid var(--line)', flexWrap: 'wrap' }}>
      {[
        ...
        { key: 'announcements', label: 'Announcements', icon: 'N' },
      ].map(t => (
            ...
            {releasedYears.length === 0 ? (
              <Card>
                <div style={{ textAlign: 'center', padding: '48px 24px' }}>
                  <div style={{ fontSize: 40, marginBottom: 16 }}>Lock</div>
                      ...
                      <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 13 }}>
                        <thead>
                          <tr style={{ borderBottom: '1px solid var(--line)' }}>
                            ...
                            {activeComps.map(c => <th key={c.key} style={thStyle}>{c.label}</th>)}
                          ...
                          {periodGrades.map(g => {
                            ...
                            return (
                              <tr key={g.id} style={{ borderBottom: '1px solid var(--line)' }}>
                                ...
                                {activeComps.map(c => <td key={c.key} style={{ ...tdStyle, color: 'var(--mist)' }}>{g[c.key] ?? '-'}</td>)}
                                <td style={{ ...tdStyle, fontWeight: 700, color: 'var(--white)' }}>{total !== null ? total.toFixed(1) : '-'}</td>
                                <td style={{ ...tdStyle }}>
                                  <Badge color={total >= 80 ? 'var(--emerald)' : total >= 60 ? 'var(--sky)' : total >= 50 ? 'var(--amber)' : 'var(--rose)'}
                                    bg={total >= 80 ? 'rgba(45,212,160,0.1)' : total >= 60 ? 'rgba(91,168,245,0.1)' : total >= 50 ? 'rgba(251,159,58,0.1)' : 'rgba(240,107,122,0.1)'}>
                                    {letter}
                                  </Badge>
                                </td>
                                <td style={{ ...tdStyle, color: 'var(--mist3)' }}>{g.position ? `${g.position}${ordinal(g.position)}` : '-'}</td>
                              </tr>
                            )
                          })}
                        </tbody>
        ...
        {tab === 'announcements' && (
          <div>
            {announcements.length === 0 ? (
              <Card>
                <div style={{ textAlign: 'center', padding: '48px 24px' }}>
                  <div style={{ fontSize: 40, marginBottom: 16 }}>Megaphone</div>
                  <div style={{ fontFamily: "'Clash Display',sans-serif", fontSize: 18, fontWeight: 700, marginBottom: 8 }}>No Announcements</div>
                  <div style={{ fontSize: 14, color: 'var(--mist3)', maxWidth: 340, margin: '0 auto', lineHeight: 1.7 }}>
                ...
                {announcements.map((a, i) => (
                  <Card key={i}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'start', gap: 12, marginBottom: 12 }}>
                      <div>
                        <div style={{ fontFamily: "'Clash Display',sans-serif", fontSize: 18, fontWeight: 700, marginBottom: 4 }}>{a.title}</div>
                        <div style={{ fontSize: 12, color: 'var(--mist3)' }}>{fmtDate(a.created_at)}</div>
                      </div>
                    </div>
                    <div style={{ fontSize: 13, color: 'var(--mist)', lineHeight: 1.6, whiteSpace: 'pre-wrap' }}>
...
return (
<div style={{ display: 'flex', flexDirection: 'column', gap: 4, paddingBottom: 12, borderBottom: '1px solid var(--line)' }}>
<div style={{ fontSize: 11, color: 'var(--mist3)', textTransform: 'uppercase', letterSpacing: '0.05em', fontWeight: 600 }}>{label}</div>
<div style={{ fontSize: 14, color: 'var(--mist)', fontFamily: mono ? "'JetBrains Mono',monospace" : 'inherit' }}>
{value || '-'}
</div>
</div>
)
...
const thStyle = { padding: '10px 12px', textAlign: 'left', fontWeight: 600, color: 'var(--mist3)', fontSize: 12, borderBottom: '1px solid var(--line)' }
const tdStyle = { padding: '10px 12px', borderBottom: '1px solid var(--line)' }
